...
class BoardListProvider extends ChangeNotifier {
  ...
  void calculateSizePosition({required int listIndex, required BuildContext context, required VoidCallback setstate}) {
  ...
  Future addNewCard({required String position, required int listIndex}) async {
    ...
    cardState.cardIndex = position == "TOP" ? 0 : prov.board.lists[listIndex].items.length - 1;
    prov.board.lists[listIndex].setState!();
  }

  void onListLongpress({required int listIndex, required BuildContext context, required VoidCallback setstate}) {
    var prov = ref.read(ProviderList.boardProvider);
              ...
              child: MediaQuery.removePadding(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (ctx, index) {
                    return Item(
                      color: prov.board.lists[listIndex].items[index].backgroundColor ?? Colors.grey.shade200,
                      itemIndex: index,
                      listIndex: listIndex,
                    );
                  },
  ...
  Future scrollToMax(ScrollController controller) async {
    ...
    await controller.animateTo(
      ...
      duration: Duration(
          milliseconds: (int.parse(controller.position.extentAfter.toString().substring(0, 3).split('.').first))),
      curve: Curves.linear,
    );
    await scrollToMax(controller);
  }
  ...
  Future scrollToMin(ScrollController controller) async {
    ...
    await controller.animateTo(
      ...
      duration: Duration(
          milliseconds: (int.parse(controller.position.extentBefore.toString().substring(0, 3).split('.').first))),
      curve: Curves.linear,
    );
    await scrollToMin(controller);
  }

  Future<void> maybeListScroll() async {
    var prov = ref.read(ProviderList.boardProvider);
    ...
    if (controller.offset < controller.position.maxScrollExtent &&
      ...
      if (prov.board.listScrollConfig == null) {
        ...
        await controller.animateTo(prov.board.listScrollConfig!.offset + controller.offset,
            duration: prov.board.listScrollConfig!.duration, curve: prov.board.listScrollConfig!.curve);
      }
      scrolling = false;
      scrollingDown = false;

      await maybeListScroll();
  ...
  void moveListRight() {
    ...
    if (prov.valueNotifier.value.dx + prov.board.lists[prov.draggedItemState!.listIndex!].width! / 2 <
        prov.board.lists[prov.draggedItemState!.listIndex! + 1].x!) {
      return;
    }
    // dev.log("LIST RIGHT");
    prov.board.lists
        .insert(prov.draggedItemState!.listIndex! + 1, prov.board.lists.removeAt(prov.draggedItemState!.listIndex!));
