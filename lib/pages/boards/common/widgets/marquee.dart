...
class _MarqueeState extends State<Marquee> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    // 不在这里启动滚动。是否滚动由 build 中根据文本是否溢出来决定。
  }
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      // 当父约束是无限大时，认为不需要滚动；否则对比文本尺寸与可用空间
      final shouldScroll = maxExtent.isFinite &&
          ((isHorizontal && textSize.width > maxExtent) || (!isHorizontal && textSize.height > maxExtent));
      ...
      if (!_running) {
        WidgetsBinding.instance.addPostFrameCallback((_) async {
          if (!_running && _controller.hasClients) {
            _running = true;
            _controller.jumpTo(_startPosition);
