...
class _AddLocationWidgetViewState extends ConsumerState<AddLocationWidgetView> {
  ...
  final buildingProvider = StateProvider<String>((ref) => "");
  final tenantProvider = StateProvider<String>((ref) => "");
  final areaProvider = StateProvider<int>((ref) => 0);
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (widget.locationModel != null) {
        ref.read(locationProvider.notifier).state = widget.locationModel!.name ?? "";
        ref.read(buildingProvider.notifier).state = widget.locationModel!.building ?? "";
        ref.read(tenantProvider.notifier).state = widget.locationModel!.tenant ?? "";
        ref.read(areaProvider.notifier).state = widget.locationModel!.area ?? 0;
      }
    });
  }
              ...
              child: Column(
                ...
                children: [
                  ...
                  CustomTitleTextField(
                    ...
                    onChanged: (value) {
                      ref.read(buildingProvider.notifier).state = value;
                    },
                  ),
                  CustomTitleTextField(
                    title: 'Tenant',
                    ...
                    onChanged: (value) {
                      ref.read(tenantProvider.notifier).state = value;
                    },
                  ),
                  CustomTitleTextField(
                    title: 'Area',
                ...
                TextButton(
                  onPressed: () async {
                    ...
                    if (widget.type == "ADD") {
                      ...
                      var result = await ref.read(addLocationProvider.notifier).updateLocation(
