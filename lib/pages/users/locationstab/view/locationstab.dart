...
class _LocationsTableState extends ConsumerState<LocationsTable> {
  ...
  Widget build(BuildContext context) {
    ...
    return ref.watch(getLocationsProvider).when(data: (data) {
      return ref.watch(itemsLocationModelFilteredProvider).length > 0
          ? DataTable2(
              ...
              rows: List<DataRow>.generate(
                ref.watch(valueSearchProviderInLocation).length > 0
                    ? ref
                            .watch(itemsLocationModelFilteredProvider.notifier)
                            .getSearchLocations(ref.watch(valueSearchProviderInLocation))
                            .length ??
                        0
                    : ref.watch(itemsLocationModelFilteredProvider).length,
                (index) => locationsDataRow(
                    (ref.watch(valueSearchProviderInLocation).length > 0
                        ? ref
                            .watch(itemsLocationModelFilteredProvider.notifier)
                            .getSearchLocations(ref.watch(valueSearchProviderInLocation))
                        : ref.watch(itemsLocationModelFilteredProvider))[index],
                    index,
                    ref),
              ),
            )
    ...
    return DataRow2.byIndex(
      ...
      onSelectChanged: (value) {
        if (locationModel.selected != value) {
          ...
          ref.read(itemsLocationModelFilteredProvider.notifier).updateLocation(locationModel);

          //单选  多选注释
          if (locationModel.selected ?? false) {
            for (var item in ref.read(itemsLocationModelFilteredProvider)) {
