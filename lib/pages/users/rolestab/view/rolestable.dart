...
class _RolesTableState extends ConsumerState<RolesTable> {
  ...
  Widget build(BuildContext context) {
    ...
    return ref.watch(getRolesProvider).when(data: (data) {
      return ref.watch(itemsRoleModelFilteredProvider).length > 0
          ? DataTable2(
              ...
              rows: List<DataRow>.generate(
                ref.watch(valueSearchProviderInRole).length > 0
                    ? ref
                            .watch(itemsRoleModelFilteredProvider.notifier)
                            .getSearchRoles(ref.watch(valueSearchProviderInRole))
                            .length ??
                        0
                    : ref.watch(itemsRoleModelFilteredProvider).length,
                (index) => rolesDataRow(
                    (ref.watch(valueSearchProviderInRole).length > 0
                        ? ref
                            .watch(itemsRoleModelFilteredProvider.notifier)
                            .getSearchRoles(ref.watch(valueSearchProviderInRole))
                        : ref.watch(itemsRoleModelFilteredProvider))[index],
                    index,
                    ref),
              ),
            )
    ...
    return DataRow2.byIndex(
      ...
      onSelectChanged: (value) {
        if (roleModel.selected != value) {
          ...
          ref.read(itemsRoleModelFilteredProvider.notifier).updateRole(roleModel);
          //单选  多选注释
          if (roleModel.selected ?? false) {
            for (var item in ref.read(itemsRoleModelFilteredProvider)) {
