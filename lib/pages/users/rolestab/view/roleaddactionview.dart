    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (_) {
        if (widget.roleModel != null) {
          ref.read(nameProvider.notifier).state = widget.roleModel!.name ?? "";
          ref.read(itemsPermissionSelectProvider.notifier).resetPermissionSelects();
          for (var item in ref.read(itemsPermissionSelectProvider)) {
            for (var item2 in widget.roleModel!.permissions ?? []) {
              if (item.module == item2.module) {
                if (item2.name == "read") {
                  ref.read(itemsPermissionSelectProvider.notifier).updatePermissionSelect(
                        PermissionSelect(
                          module: item.module,
                          read: true,
                          write: item.write,
                  ...
                  onPressed: () async {
                    ...
                    if (name.isEmpty) {
                      ...
                      if (widget.type == "ADD") {
                        ...
                        for (var item in ref.read(itemsPermissionSelectProvider)) {
                          for (var item2 in ref.read(itemsPermissionProvider)) {
                            if (item.module == item2.module) {
                        ...
                        bool result = await ref.read(addRoleProvider.notifier).updateRole(
                            widget.roleModel?.id,
                            AddRoleRequestEntity(
                              name: name,
                              permissionIds: permissionIds,
