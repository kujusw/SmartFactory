...
class BallSpinFadeLoaderIndicator extends Indicator {
  ...
  paint(Canvas canvas, Paint paint, Size size) {
    ...
    for (int i = 0; i < 8; i++) {
      canvas.save();
      Offset point = circleAt(size.width, size.height, size.width / 2 - radius, i * (pi / 4));
  ...
  List<AnimationController> animation() {
    ...
    for (int i = 0; i < 8; i++) {
      var controller = new AnimationController(duration: Duration(milliseconds: 500), vsync: context);
