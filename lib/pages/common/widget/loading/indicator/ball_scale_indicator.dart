...
class BallScaleIndicator extends Indicator {
  ...
  paint(Canvas canvas, Paint paint, Size size) {
    ...
    canvas.drawCircle(Offset(width / 2, height / 2), (width / 2 - circleSpacing) * mScale, paint);
  }
  ...
  List<AnimationController> animation() {
    ...
    AnimationController alphaController =
        new AnimationController(duration: const Duration(milliseconds: 1000), vsync: context);
    Animation<int> alpha = new IntTween(begin: 255, end: 0).animate(alphaController);
    alpha.addListener(() {
      mAlpha = alpha.value;
      postInvalidate();
    });
